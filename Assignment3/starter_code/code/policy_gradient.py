...
class PolicyGradient(object):
    ...
    def init_policy(self):
        ...
        network = build_mlp(input_size=self.observation_dim, output_size=self.action_dim, n_layers=self.config.n_layers, size=self.config.layer_size)
        if self.discrete:
    ...
    def get_returns(self, paths):
        ...
        for path in paths:
            ...
            for reward in reversed(rewards[:-1]):
                returns.insert(0, reward + self.config.gamma * returns[0])
            #######################################################
            #########          END YOUR CODE.          ############
    ...
    def update_policy(self, observations, actions, advantages):
        ...
        act_dist = self.policy.action_distribution(observations)
        policy_performance = act_dist.log_prob(actions) * advantages
        loss = -1 * policy_performance.mean()
