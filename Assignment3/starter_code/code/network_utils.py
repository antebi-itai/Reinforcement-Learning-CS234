...
def build_mlp(input_size, output_size, n_layers, size):
    ...
    net = []
    net += [nn.Linear(in_features=input_size, out_features=size), nn.ReLU()]
    for _ in range(n_layers - 1):
        net += [nn.Linear(in_features=size, out_features=size), nn.ReLU()]
    net.append(nn.Linear(in_features=size, out_features=output_size))
